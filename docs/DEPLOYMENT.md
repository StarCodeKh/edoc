...
- **`vendor/` and `node_modules/` are committed.** A `git pull` brings the PHP and
  JS dependencies with it, so `composer install` and `npm install` are usually
  not needed. It also means a rollback restores the matching dependencies.
- **`public/build` is *not* committed.** The compiled CSS and JS never arrive
  with a pull. **Every deploy must run `npm run build` on the server**, or the
  site comes up blank.
...
> **The web PHP and the CLI PHP are often different versions.** What matters is
> the one PHP-FPM runs. Confirm it from the browser's side:
> ```bash
...
# 3. pull (brings vendor/ and node_modules/ with it)
git pull --ff-only origin main

# 4. build the front end — public/build is NOT in the repo
npm run build

# 5. a leftover dev-server marker blanks every page
rm -f public/hot

# 6. database
php artisan migrate --force

# 7. caches: clear before rebuilding
...
**Step 7 is the one people skip.** A Blade view compiled by an older release
...
Because `vendor/` is committed, a reset restores the code *and* its
dependencies:

```bash
cd /var/www/html/edoc
git reset --hard <commit>
npm run build          # assets must match the code you rolled back to
...
| Symptom | Cause | Fix |
|---|---|---|
| **`ERR_EMPTY_RESPONSE`**, nothing in the browser | PHP died before writing a byte — almost always the PHP version being below what the release needs | `php -v`; check the FPM and nginx error logs; roll back |
...
| **500 on every page** | `storage/` or `bootstrap/cache` not writable | re-run the permissions in §3 |
...
sudo tail -50 /var/log/nginx/edoc-error.log
