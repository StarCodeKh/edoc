<template>
    <div class="h-full" :class="{ right_menu_enable: show_right_menu }">
        ...
        <board-view-menu
            ...
            @menu-toggle="show_right_menu = !show_right_menu"
            @fClear="reset()"
            :filters="filters"
            view="board"
        />
            ...
            <div v-else class="board_width" :class="{ v_label: showLabelName }">
                    ...
                    <div class="b__list">
                        <div class="flex w-full text-sm font-semibold">
                            <span
                                ...
                                @keypress="saveListTitle($event, listItem.id)"
                                @blur="saveListTitle($event, listItem.id)"
                                >{{ listItem.title }}</span
                            >
                        </div>
                        <span
                            ...
                            >{{ getDoneCount(listItem) + '/' + listItem.tasks.length }}</span
                        >
                        <button
                            @click="listItem.show_more = !listItem.show_more"
                        ...
                        <div
                            v-if="listItem.show_more"
                            class="absolute right-9 top-2 w-30 z-999 bg-white py-3 rounded shadow"
                        >
                            <button
                                v-if="listIndex !== 0"
                                @click="
                                    moveList(listIndex, 'minus');
                                    listItem.show_more = false;
                                "
                                class="flex w-full items-center hover:bg-gray-200 px-3 py-2 text-xs font-medium focus:outline-none focus:ring-0"
                            >
                            ...
                            <button
                                @click="makeListArchive($event, listItem.id, listIndex)"
                                class="flex w-full items-center hover:bg-gray-200 px-3 py-2 text-xs font-medium focus:outline-none focus:ring-0"
                            >
                    ...
                    >
                        <template #item="{ element, index }">
                            <div
                                ...
                                draggable="true"
                                :class="{ 'is-selected-for-merge': selectedTaskIds.includes(element.id) }"
                            >
                                <label
                                    v-if="taskCan('edit', element)"
                                    class="task-select-checkbox"
                                    :class="{ 'task-select-checkbox--checked': selectedTaskIds.includes(element.id) }"
                                    @click.stop
                                >
                                    <input
                                        type="checkbox"
                                        :checked="selectedTaskIds.includes(element.id)"
                                        @change="toggleTaskSelect(element.id)"
                                    />
                                    <icon name="tick_check" class="w-3 h-3" />
                                </label>
                                <div v-if="element.show_more" class="absolute right-7 top-1 w-30 z-999 bg-gray-100">
                                    <button
                                        v-if="element.is_done"
                                    ...
                                    <button
                                        v-if="element.is_done"
                                        @click="saveTask(element.id, { is_done: false })"
                                        class="m__archive"
                                    >
                                    ...
                                    <button
                                        v-if="!element.is_done"
                                        @click="saveTask(element.id, { is_done: true })"
                                        class="m__archive"
                                    >
                                ...
                                <button
                                    v-if="taskCan('edit', element)"
                                    @click="visibleShowMore($event, element)"
                                ...
                                <div
                                    v-if="element.cover"
                                    @click="cardPrimaryClick(element)"
                                    class="t__cover"
                                    :style="{
                                        backgroundImage: 'url(' + element.cover.path + ')',
                                        height: element.cover.width
                                            ? element.cover.height / (element.cover.width / 246) + 'px'
                                            : 'auto',
                                    }"
                                ></div>
                                ...
                                <div class="t__details" @click="cardPrimaryClick(element)">
                                    ...
                                    <div class="t__title__area">
                                        <div
                                            v-if="element.is_done"
                                            class="checklist-box"
                                            @click="cardSwitchClick($event)"
                                        >
                                            <input
                                                type="checkbox"
                                                :checked="!!element.is_done"
                                                @change="cardSwitchToggle(element, $event)"
                                            />
                                    ...
                                    <div
                                        ...
                                        class="t__priority"
                                        :style="{ '--p-color': element.priority.color || '#9ca3af' }"
                                    >
                                    ...
                                    <div
                                        v-if="hasBeenMerged(element)"
                                        class="merged-note"
                                        :title="$t('This task has been merged — click to view')"
                                        @click.stop="openDrawer(element)"
                                    >
                                        <svg viewBox="0 0 24 24" fill="none" class="merged-note__icon w-3 h-3">
                                            <path
                                                d="M6 3v10a4 4 0 0 0 4 4h4M6 3L3 6m3-3l3 3M18 21v-4M18 21l-3-3m3 3l3-3"
                                                stroke="currentColor"
                                                stroke-width="1.8"
                                                stroke-linecap="round"
                                                stroke-linejoin="round"
                                    ...
                                    <div class="card__footer" @click="cardPrimaryClick(element)">
                                        <div
                                            ...
                                            aria-label="Due date"
                                            class="__item due"
                                            :class="getDue(element)"
                                        >
                                            <icon class="w-4 h-4" name="time" />
                                            <span class="pl-[2px] pr-[4px] leading-none">
                                        ...
                                        <div
                                            ...
                                            :class="{
                                                completed: element.checklist_done_count === element.checklists_count,
                                            }"
                                        >
                            ...
                            <div class="add_new pt-1">
                                ...
                                <div class="mb-2" v-show="listItem.new_task_open">
                                    <input
                                        ...
                                        @keyup="$event.keyCode === 13 ? submitNewTask(listItem, listIndex) : ''"
                                    />
                                    <div class="pl-1 mt-2 flex">
                                        <button
                                            @click="submitNewTask(listItem, listIndex)"
                                        ...
                                        <button
                                            @click="listItem.new_task_open = false"
                ...
                <div class="flex flex-col w-72 add__new__list">
                    <div class="add_new" :class="{ active: new_list_open }">
                        <div
                            v-if="!new_list_open"
                            class="group p-3 flex cursor-pointer items-center rounded"
                            @click="openNewList()"
                        >
                        ...
                        <div class="p-3" v-show="new_list_open">
                            <input
                                autofocus
                                type="text"
                                :id="'new_list_input_id_' + lists.length"
                                :ref="'new_list_input_' + lists.length"
                                v-model="new_list.title"
                                class="block text-sm font-medium w-full px-2 py-2 rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                placeholder="Enter list title..."
                                @keyup="$event.keyCode === 13 ? submitNewList($event) : ''"
                            />
                            <div class="mt-2 flex">
                                <button
                                    @click="submitNewList($event)"
                                    class="inline-flex items-center border font-medium shadow-sm text-white border-transparent bg-indigo-600 hover:bg-indigo-700 px-2.5 py-1.5 text-xs rounded"
                                >
                                    Add list
                                </button>
                                <button
                                    @click="new_list_open = false"
                                    class="inline-flex items-center border font-medium shadow-sm text-gray-700 border-gray-300 bg-white hover:bg-gray-50 px-2.5 py-1 text-xs rounded ltr:ml-1 rtl:mr-1"
                                >
                                    Cancel
                                </button>
                            </div>
            ...
            <div class="doc-preview-shell">
                ...
                <div class="doc-preview-shell__body">
                    <div class="doc-preview-card">
                        <div class="doc-preview-card__eyebrow">
                            {{ $t('Task board') }} · {{ $t('Merge consolidation') }}
                        </div>
                        ...
                        <div class="doc-preview-card__meta">
                            <div>
                                <span class="doc-preview-card__meta-label">{{ $t('Ref. No.') }}</span>
                                <span class="doc-preview-card__meta-value">{{ mergeRefNo }}</span>
                            </div>
                            <div>
                                {{ $t('Date') }}:
                                <span class="doc-preview-card__meta-value">{{ moment().format('DD MMM, HH:mm') }}</span>
                            </div>
                        </div>
                        ...
                        <table class="doc-preview-table">
                            ...
                            <tbody>
                                <tr
                                    v-for="(t, i) in selectedTasksDetails"
                                    :key="t.id"
                                    :class="{ 'doc-preview-table__row--kept': mergeTargetId === t.id }"
                                >
                                    <td>{{ i + 1 }}</td>
                                    <td>{{ t.title }}</td>
                                    ...
                                    <td>
                                        <label class="doc-preview-radio">
                                            ...
                                            />
                                            <span>{{
                                                mergeTargetId === t.id ? $t('Keep this one') : $t('Merge away')
                                            }}</span>
                                        </label>
                                    </td>
                                </tr>
                        ...
                        <p class="doc-preview-card__note">
                            {{ $t('Generated automatically by the task board. Consolidates') }}
                            {{ selectedTasksDetails.length }}
                            {{
                                $t(
                                    'task(s) into one record — comments, attachments and checklist items combine into the kept task.'
                                )
                            }}
                        </p>
                        ...
                        <div class="doc-preview-barcode">
                            <div class="doc-preview-barcode__bars">
                                <span
                                    v-for="n in 46"
                                    :key="n"
                                    :style="{ width: (n % 5 === 0 ? 3 : 1.5) + 'px' }"
                                ></span>
                            </div>
                            <div class="doc-preview-barcode__code">
                                {{ mergeTargetId ? documentCode(findTaskById(mergeTargetId) || {}) : mergeRefNo }}
                            </div>
                        </div>
                    </div>
                </div>
                <div class="doc-preview-shell__footer">
                    <button type="button" class="doc-preview-btn doc-preview-btn--ghost" @click="closeMergeModal()">
                        {{ $t('Cancel') }}
                    </button>
                    <button
                        type="button"
                        class="doc-preview-btn doc-preview-btn--primary"
            ...
            <div v-if="drawerOpen" class="doc-drawer-backdrop" @click.self="closeDrawer()">
                <div class="doc-drawer">
                    <div class="doc-drawer__header">
                        ...
                        <div class="doc-drawer__ref">
                            {{ drawerTask ? documentCode(drawerTask) : '' }} · {{ $t('Task Document') }}
                        </div>
                        ...
                        <span
                            v-if="drawerTask && latestMergeCode(drawerTask)"
                            class="doc-drawer__badge doc-drawer__badge--code"
                            >{{ latestMergeCode(drawerTask) }}</span
                        >
                    </div>
                    ...
                    <div class="doc-drawer__body">
                        ...
                        <div class="doc-drawer__grid">
                            <div class="doc-drawer__field">
                                ...
                                <span class="doc-drawer__field-value">{{
                                    drawerTask && drawerTask.created_at
                                        ? moment(drawerTask.created_at).format('DD MMM, HH:mm')
                                        : 'N/A'
                                }}</span>
                            </div>
                            <div class="doc-drawer__field">
                                ...
                                <span class="doc-drawer__field-value">{{
                                    drawerTask && drawerTask.updated_at
                                        ? moment(drawerTask.updated_at).format('DD MMM, HH:mm')
                                        : 'N/A'
                                }}</span>
                            </div>
                            <div class="doc-drawer__field">
                                ...
                                <span class="doc-drawer__field-value">{{
                                    (drawerTask && drawerTask.priority && drawerTask.priority.name) || $t('Normal')
                                }}</span>
                            </div>
                            <div class="doc-drawer__field doc-assign">
                                <span class="doc-drawer__field-label">
                                    {{ $t('Assigned') }}
                                    <button
                                        v-if="taskCan('edit', drawerTask)"
                                        type="button"
                                        class="doc-assign__add"
                                        :title="$t('Assignee')"
                                ...
                                <div v-if="drawerAssignees.length" class="doc-assign__list">
                                    <span
                                        v-for="a in drawerAssignees"
                                        :key="'da_' + a.user_id"
                                        class="doc-assign__chip"
                                    >
                                        <img
                                            class="doc-assign__avatar"
                                            :src="(a.user && a.user.photo_path) || '/images/user.svg'"
                                            :alt="a.user && a.user.name"
                                        />
                                        <span class="doc-assign__meta">
                                            <span class="doc-assign__name">{{ a.user && a.user.name }}</span>
                                            <span v-if="a.user && a.user.title" class="doc-assign__title">{{
                                                a.user.title
                                            }}</span>
                                        </span>
                                    </span>
                                </div>
                                <span v-else class="doc-drawer__field-value">{{
                                    '— ' + $t('not yet assigned') + ' —'
                                }}</span>
                                ...
                                <div v-if="assignPickerOpen" class="doc-assign__picker" @click.stop>
                                    <div class="doc-assign__head">
                                        ...
                                        <div class="doc-assign__head-text">
                                            <div class="doc-assign__head-row">
                                                ...
                                                <span v-if="drawerAssignees.length" class="doc-assign__head-count">{{
                                                    drawerAssignees.length
                                                }}</span>
                                            </div>
                                    ...
                                    <ul class="doc-assign__options">
                                        ...
                                        <li v-for="(m, mi) in filteredAssignMembers" :key="'am_' + m.user_id">
                                            <label class="doc-assign__option" :for="'dw_u_' + mi">
                                                <input
                                                    :id="'dw_u_' + mi"
                                                    type="checkbox"
                                                    class="doc-assign__cb"
                                                    :checked="drawerAssigneeIds.includes(Number(m.user_id))"
                                                    @change="toggleAssignee($event.target.checked, m.user_id)"
                                                />
                                                <img
                                                    class="doc-assign__avatar"
                                                    :src="(m.user && m.user.photo_path) || '/images/user.svg'"
                                                    :alt="m.user && m.user.name"
                                                />
                                                <span class="doc-assign__meta">
                                                    <span class="doc-assign__name">{{ m.user && m.user.name }}</span>
                                                    <span v-if="m.user && m.user.title" class="doc-assign__title">{{
                                                        m.user.title
                                                    }}</span>
                                                </span>
                                            </label>
                                        ...
                                        <li
                                            v-if="!assignLoading && !filteredAssignMembers.length"
                                            class="doc-assign__empty"
                                        >
                                            {{ $t('No item found!') }}
                                        </li>
                                    </ul>
                        ...
                        <div class="doc-drawer__section-label-row">
                            <span class="doc-drawer__section-label">
                                {{ drawerTask && hasBeenMerged(drawerTask) ? $t('Combined from') : $t('Document') }} ·
                            ...
                            <div v-for="(doc, i) in drawerDocuments" :key="doc.id" class="doc-drawer__docrow">
                                <button type="button" class="doc-drawer__docrow-main" @click="openDocDetail(i)">
                                    ...
                                    <svg
                                        viewBox="0 0 24 24"
                                        fill="none"
                                        class="doc-drawer__docrow-chevron doc-drawer__docrow-chevron--arrow"
                                    >
                                        <path
                                            d="M9 6l6 6-6 6"
                                            stroke="currentColor"
                        ...
                        >
                            ...
                            <svg v-else viewBox="0 0 24 24" fill="none" class="sign-request-btn__icon">
                                <path
                                    d="M3 20h18M5.5 16.5l9.9-9.9a1.6 1.6 0 0 1 2.3 0l.7.7a1.6 1.6 0 0 1 0 2.3l-9.9 9.9-3.6.6.6-3.6z"
                                    stroke="currentColor"
                                    stroke-width="1.7"
                                    stroke-linecap="round"
                                    stroke-linejoin="round"
                            ...
                            <span class="sign-request-btn__text">{{
                            ...
                            <div class="doc-drawer__timeline-item" v-for="a in drawerActivities" :key="a.id">
                                ...
                                <div class="doc-drawer__timeline-card">
                                    <div class="doc-drawer__timeline-top">
                                        ...
                                        <span class="doc-drawer__timeline-time">{{
                                    ...
                                    <div class="doc-drawer__timeline-bottom">
                                        <span v-if="a.user && a.user.name" class="doc-drawer__timeline-user">
                                            <span class="doc-drawer__timeline-avatar">{{
                                                (a.user.name || '?').charAt(0).toUpperCase()
                                            }}</span>
                                            {{ a.user.name }}
                                        </span>
                                        <span class="doc-drawer__timeline-abs">{{
                                            moment(a.created_at).format('DD MMM, HH:mm')
                                        }}</span>
                                    </div>
                                </div>
                            </div>
        ...
        <div
            v-if="docDetailOpen && currentDocDetail"
            class="fixed inset-0 z-[560] bg-black/40 flex items-center justify-center p-4"
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
                <div class="flex items-start justify-between px-5 pt-5 pb-3">
                    <div class="flex items-start gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            <h3 class="text-base font-bold text-gray-900 leading-snug truncate">
                                {{ currentDocDetail.title }}
                            </h3>
                            <p class="text-xs text-gray-500 mt-0.5">
                                {{ $t('Document') }} {{ drawerExpandedIndex + 1 }} {{ $t('of') }}
                                {{ drawerDocuments.length }}
                            </p>
                        </div>
                    </div>
                    <button
                        ...
                        class="w-7 h-7 rounded-full flex items-center justify-center text-gray-400 hover:bg-gray-100 hover:text-gray-600 flex-shrink-0"
                    >
                        <icon name="close" class="w-4 h-4" />
                    </button>
                </div>
                ...
                <div class="px-5 pb-2 divide-y divide-gray-100">
                    ...
                    <div class="flex items-center justify-between py-2.5">
                        ...
                        <span class="text-sm font-medium text-gray-900">{{
                            currentDocDetail.kept ? $t('Kept task') : $t('Merged in')
                        }}</span>
                    </div>
                    ...
                    <div v-if="currentDocDetail.merged_at" class="flex items-center justify-between py-2.5">
                        ...
                        <span class="text-sm font-medium text-gray-900">{{
                            moment(currentDocDetail.merged_at).format('DD MMM, HH:mm')
                        }}</span>
                    </div>
                </div>
                ...
                <div v-if="!currentDocDetail.kept" class="px-5 pb-2">
                    ...
                    >
                        <svg viewBox="0 0 24 24" fill="none" class="w-3.5 h-3.5">
                            <path
                                d="M9 3H5a2 2 0 0 0-2 2v4m18 0V5a2 2 0 0 0-2-2h-4M9 21H5a2 2 0 0 1-2-2v-4m18 0v4a2 2 0 0 1-2 2h-4"
                                stroke="currentColor"
                                stroke-width="1.8"
                                stroke-linecap="round"
                                stroke-linejoin="round"
...
export default {
    ...
    watch: {
        form: {
            ...
            handler: throttle(function () {
                this.$inertia.get(
                    this.route('projects.view.board', this.project.slug || this.project.id),
                    pickBy(this.form),
                    { preserveState: true }
                );
            }, 150),
    ...
    computed: {
        selectedTasksDetails() {
            return this.selectedTaskIds.map((id) => this.findTaskById(id)).filter(Boolean);
        },
        drawerAssignees() {
    ...
    methods: {
        ...
        openSignatureRequest(task) {
            ...
            axios
                .get(this.route('task.signature.request.show', { taskUid: task.id }))
                .then(({ data }) => {
                    if (!data.attachment) {
        ...
        auditIconBg(a) {
            const field = a && a.field_changed;
            if (field === 'is_done')
                return this.auditTurnedOn(a) ? 'doc-drawer__timeline-icon--green' : 'doc-drawer__timeline-icon--gray';
            if (field === 'is_archive')
                return this.auditTurnedOn(a) ? 'doc-drawer__timeline-icon--amber' : 'doc-drawer__timeline-icon--gray';

            const map = {
                title: 'doc-drawer__timeline-icon--blue',
                slug: 'doc-drawer__timeline-icon--blue',
                list_id: 'doc-drawer__timeline-icon--sky',
                order: 'doc-drawer__timeline-icon--sky',
                ...
                comment: 'doc-drawer__timeline-icon--gold',
                comment_edit: 'doc-drawer__timeline-icon--gold',
            ...
            axios
                .get(
                    this.route('task.other.data', {
                        ...
                        project_id: this.drawerTask.project_id,
                    })
                )
                .then((response) => {
        ...
        toggleAssignee(checked, userId) {
            if (!this.drawerTask) return;
            axios
                .post(this.route('task.assignees.add'), { task_id: this.drawerTask.id, user_id: userId })
                .then((response) => {
                    ...
                    const added = response.data && response.data.action ? response.data.action === 'assigned' : checked;
        ...
        fetchDrawerActivities(taskId) {
            ...
            this.drawerActivities = [];
            axios
                .get(this.route('task.activities', taskId))
                .then((response) => {
        ...
        getDue(element) {
            return element.is_done
                ? 'done'
                : moment().isAfter(element.due_date)
                  ? 'over_due'
                  : moment(element.due_date).isBetween(moment(), moment().add(1, 'day'))
                    ? 'due_soon'
                    : '';
        },
...
.dark .doc-assign__name {
    color: #f3f4f6;
}
.dark .doc-assign__title {
    color: #9ca3af;
}
...
.doc-assign__close:hover {
    background: #f3f4f6;
}
.dark .doc-assign__close:hover {
    background: #374151;
}
.doc-assign__search {
    width: 100%;
...
.doc-assign__option:hover {
    background: #f3f4f6;
}
.dark .doc-assign__option:hover {
    background: #374151;
}
.doc-assign__cb {
    width: 16px;
...
.doc-drawer {
    ...
    display: flex;
    flex-direction: column;
    /* The panel holds its shape; only the body below the header scrolls,
           so the title and the close button stay reachable all the way down. */
    overflow: hidden;
}
.doc-drawer__header {
    position: relative;
...
.doc-drawer__timeline-icon--blue {
    background: #3b82f6;
}
.doc-drawer__timeline-icon--sky {
    background: #0ea5e9;
}
.doc-drawer__timeline-icon--orange {
    background: #f97316;
}
.doc-drawer__timeline-icon--green {
    background: #22c55e;
}
.doc-drawer__timeline-icon--amber {
    background: #f59e0b;
}
...
.doc-drawer__timeline-icon--gold {
    background: #b8860b;
}
.doc-drawer__timeline-icon--red {
    background: #ef4444;
}
.doc-drawer__timeline-icon--gray {
    background: #9ca3af;
}
