                                ...
                                <template #item="{ element, index }">
                                    ...
                                    >
                                        ...
                                        <div class="text-sm" :data-label="$t('ថ្ងៃឯកសារចូល')">
                                            {{
                                                element.entry_date
                                                    ? moment(element.entry_date).format('DD MMM YYYY')
                                                    : ''
                                            }}
                                        </div>
                                ...
                                <li v-for="(listObject, li_index) in searchList(list_search)">
                                    <label class="flex p-2 cursor-pointer hover:bg-gray-200 rounded">
                                        <input
                                            ...
                                            @change="saveTask(selected.task_id, { list_id: listObject.id })"
                                        />
...
export default {
    ...
    watch: {
        form: {
            ...
            handler: throttle(function () {
                this.$inertia.get(
                    this.route('projects.view.table', this.project.slug || this.project.id),
                    pickBy(this.form),
                    { preserveState: true }
                );
            }, 150),
    ...
    methods: {
        ...
        taskDetails(id) {
            if (id) {
                window.location.href = this.route('projects.table.with.task', {
