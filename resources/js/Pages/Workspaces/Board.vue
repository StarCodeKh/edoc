                                        ...
                                        <div
                                            ...
                                            :style="[
                                                element.project.background
                                                    ? { background: 'url(' + element.project.background.image + ')' }
                                                    : {},
                                            ]"
                                        ></div>
                ...
                <div class="flex flex-col w-72 add__new__list">
                    <div class="add_new" :class="{ active: new_list_open }">
                        ...
                        <div class="p-3" v-show="new_list_open">
                            <select
                                v-model="new_list.project_id"
                                class="block text-sm font-medium w-full px-2 py-2 mb-2 rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            >
                                <option value="">{{ $t('Select Project') }}</option>
                                <option v-for="project in workspaceProjects" :key="project.id" :value="project.id">
                                    {{ project.title }}
                                </option>
                            </select>
                            <input
...
export default {
    ...
    watch: {
        form: {
            ...
            handler: throttle(function () {
                this.$inertia.get(
                    this.route('workspace.view.board', this.workspace.slug || this.workspace.id),
                    pickBy(this.form),
                    { preserveState: true }
                );
            }, 150),
    ...
    methods: {
        ...
        handleExportTasks() {
            ...
            if (projectIds) {
                ...
                window.location.href = this.route(
                    'project.csv.export',
                    this.workspaceProjects[0]?.id || this.workspace.id
                );
            } else {
        ...
        loadWorkspaceProjects() {
            axios
                .get(this.route('json.projects.all', this.workspace.id))
                .then((response) => {
                    if (response.data) {
            ...
            if (this.new_list.title && this.new_list.project_id) {
                axios
                    .post(this.route('json.list.add'), {
                        project_id: this.new_list.project_id,
        ...
        makeListArchive(e, id, index) {
            ...
            if (listItem && listItem.list_ids && listItem.list_ids.length > 1) {
                Promise.all(listItem.list_ids.map((listId) => axios.post(this.route('json.list.archive', listId))))
                    .then(() => {
        ...
        changeBoardTitle(id, title) {
            ...
            if (listItem && listItem.list_ids && listItem.list_ids.length > 1) {
                ...
                Promise.all(
                    listItem.list_ids.map((listId) => axios.post(this.route('board.update', listId), { title }))
                )
                    .then(() => {
        ...
        afterDrop(e) {
            ...
            if (!!e.pullMode) {
                ...
                const actualListId =
                    targetList && targetList.list_ids && targetList.list_ids.length > 0
        ...
        submitNewTask(listItem, listIndex) {
            if (this.new_task.title) {
                ...
                const actualListId =
                    listItem.list_ids && listItem.list_ids.length > 0 ? listItem.list_ids[0] : listItem.id;
                let task = {
