                ...
                <div class="inline-block min-w-full h-full py-4 align-middle md:px-3 lg:px-4">
                    <div class="table__view">
                        <table>
                            ...
                            <draggable
                                v-for="(listItem, listIndex) in lists"
                                :key="listItem.id"
                            ...
                            >
                                <template #item="{ element, index }">
                                    <tr class="list-group-item group" :data-id="element.id">
                                        <td
                                            class="pl-2 pr-1 py-2 border-none text-sm font-medium whitespace-nowrap w-[20px]"
                                        >
                                            <div
                                            ...
                                            <Link
                                                ...
                                                :href="
                                                    this.route('projects.view.board', {
                                                        uid: element.project.slug || element.project.id,
                                                        task: element.slug || element.id,
                                                    })
                                                "
                                                :data-id="element.id"
                                        ...
                                        >
                                            <div class="t__title" v-if="element.due_date">
                                                <DatePicker
                                                    ...
                                                    @change="
                                                        saveTask(element.id, { due_date: element.due_date }, listIndex)
                                                    "
                                                >
                                                    <template #trigger>
    ...
    watch: {
        form: {
            ...
            handler: throttle(function (newVal, oldVal) {
                ...
                if (
                    oldVal &&
                    oldVal.due === newVal.due &&
                    oldVal.label === newVal.label &&
                    oldVal.task === newVal.task
                ) {
                    return;
                }
                ...
                this.$inertia.get(
                    this.route('workspace.view.my-tasks.table', this.workspace.slug || this.workspace.id),
                    filters,
                    { preserveState: true }
                );
            }, 150),
