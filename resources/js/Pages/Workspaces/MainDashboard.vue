...
export default {
    ...
    props: {
        ...
        // statusCards / summary / statistics can still be passed in from
        // the backend if you want curated/categorized numbers (e.g. split
        // by document type). Left empty by default now — previously these
        // defaulted to hardcoded demo numbers (233 total, 12/15/20/10,
        // etc.) that never changed no matter which workspace you were on,
        // which is exactly what made the dashboard look "static". With no
        // override, the computed resolved* values below derive real
        // numbers from `lists` (this workspace's actual board/tasks data)
        // instead.
    ...
    data() {
        return {
            ...
            // used on the board and full table views.
            receiptModalOpen: false,
            selectedReceiptTask: null,
            // Pagination: taskRows holds the full sorted set; pageRows is
            // just the current page's slice and is what <draggable> binds
            // to (see afterDrop() for how a drag on this page gets folded
            // back into taskRows/order).
            currentPage: 1,
            pageSize: 10,
            pageRows: [],
            // Shared color cycle for both the status-card rings and the
            // summary donut segments, keyed off each list's position —
            // same idea used in the Table view so colors stay consistent
            // across the app instead of a fixed "first is green" rule.
    ...
    computed: {
        ...
        // Admins (workspace.member.role === 'admin') see every document
        // in the workspace; anyone else only sees documents they're
        // actually assigned to — same admin check the sidebar already
        // uses to decide whether to show "Workspace Tasks". Every count
        // below (cards, donut, statistics, the table itself) is built
        // from tasksForList()/allTasks so they all stay consistent with
        ...
        // Builds one real "Document Status" card straight from this
        // workspace's board lists (each list = one ring/segment), with
        // the actual (role-filtered) task counts instead of fixed demo
        // numbers.
        dynamicStatusCard() {
        ...
        // Falls back to the real, computed card only when the backend
        // hasn't supplied its own statusCards prop.
        resolvedStatusCards() {
            return (this.statusCards && this.statusCards.length) ? this.statusCards : [this.dynamicStatusCard];
        },

        // Real overall completion percent + one segment per list, built
        // from actual (role-filtered) task counts.
        dynamicSummary() {
        ...
        // Per-list done/total breakdown, reusing the same (role-filtered)
        // source data as the status card above. If your backend tracks a
        // different breakdown (e.g. per department across multiple
        // workspaces), pass a `statistics` prop and this is skipped
        // entirely.
        dynamicStatistics() {
    ...
    methods: {
        ...
        // page number first in case the set got smaller (e.g. a task was
        // archived) and the old page no longer exists.
        syncPageRows() {
        ...
        afterDrop() {
            ...
            // from this page's offset so order stays correct relative to
            // every other page, then fold the new order back into the
            // full (unpaginated) taskRows so switching pages doesn't lose it.
            const start = (this.currentPage - 1) * this.pageSize;
        ...
        // Single source of truth for "which tasks in this list is the
        // current user allowed to see" — admins get everything, everyone
        // else only what's assigned to them.
