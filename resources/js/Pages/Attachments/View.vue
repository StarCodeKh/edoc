<template>
    ...
    <div class="flex flex-col h-screen bg-gray-100 dark:bg-gray-900">
        ...
        <!-- ================= Header ================= -->
        <header class="flex items-center gap-3 px-3 sm:px-5 py-2.5 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
            <a
                v-if="backUrl"
                :href="backUrl"
            ...
            >
                <svg viewBox="0 0 24 24" fill="none" class="w-5 h-5"><path d="M15 6l-6 6 6 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </a>

            <span class="hidden sm:flex items-center justify-center w-10 h-10 rounded-lg bg-red-50 dark:bg-red-500/10 text-[10px] font-bold tracking-wide text-red-600 dark:text-red-400 flex-shrink-0">
                {{ fileExtension }}
            </span>
            ...
            <div class="min-w-0 flex-1">
                ...
                <p class="truncate text-[11px] text-gray-500 dark:text-gray-400">
                    ...
                    <span v-if="attachment"> &middot; {{ moment(attachment.created_at).format('MMM D, YYYY [at] h:mm A') }}</span>
            ...
            <div v-if="task && task.id" class="relative hidden md:flex items-center gap-2 flex-shrink-0" v-click-outside="closeAssigneeBox">
                <div class="flex -space-x-2">
                    <span v-for="assignee in (task.assignees || [])" :key="assignee.id" :title="assignee.user.name" class="block rounded-full h-8 w-8 border-2 border-white dark:border-gray-800">
                ...
                <button type="button" @click="showAssigneeBox = !showAssigneeBox" class="w-8 h-8 rounded-full flex items-center justify-center border border-dashed border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" :title="$t('Assignees')">
                    <icon class="h-4 w-4" name="add" />
                </button>

                <div v-if="showAssigneeBox" class="absolute right-0 top-11 z-30 flex w-[300px] text-sm flex-col bg-white dark:bg-gray-800 px-4 py-4 rounded-lg shadow-xl ring-1 ring-black/5 dark:ring-white/10">
                    ...
                    <input v-model="user_search" class="border-[2px] px-2 py-1 border-gray-400 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-[3px] dark:placeholder-gray-400" :placeholder="$t('Search User')" />
                    <ul class="flex flex-col mt-3 gap-1 h-56 max-h-56 overflow-y-auto scroll-smooth overscroll-contain">
                        <li v-for="(userObject, user_index) in searchUser(user_search)" :key="'assignee_'+user_index">
                            <label :for="'doc_u_id_'+user_index" class="flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-600 rounded">
                                <input :id="'doc_u_id_'+user_index" class="w-5 flex-shrink-0" type="checkbox" :checked="task_assignees().includes(userObject.user_id)" @change="assignUserToTask($event.target.checked, userObject.user_id)">
                                <img v-if="userObject.user.photo_path" :alt="userObject.user.name" class="w-6 h-6 rounded-full flex-shrink-0" :src="userObject.user.photo_path" />
                                <img v-else :alt="userObject.user.name" class="w-6 h-6 rounded-full flex-shrink-0" src="/images/user.svg" />
        ...
        <div v-if="loading" class="flex-1 flex items-center justify-center">
            <div class="w-9 h-9 rounded-full border-2 border-gray-300 dark:border-gray-600 border-t-gray-700 dark:border-t-gray-200 animate-spin"></div>
        </div>
        ...
        <div v-else-if="!attachment" class="flex-1 flex flex-col items-center justify-center gap-3 px-6 text-center">
            ...
            <a v-if="backUrl" :href="backUrl" class="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline">{{ $t('Back to task') }}</a>
        </div>

        <!-- ================= Image preview ================= -->
        <div v-else-if="isImage(attachment.name)" class="flex-1 overflow-auto flex items-center justify-center p-6">
            <img :src="attachment.path" :alt="attachment.name" class="max-w-full max-h-full object-contain rounded shadow-lg" />
        </div>

        <!-- ================= Anything we cannot preview ================= -->
        <div v-else-if="!isPdf(attachment.name)" class="flex-1 flex flex-col items-center justify-center gap-3 px-6 text-center">
            <icon name="attachment" class="w-10 h-10 text-gray-400" />
            <p class="text-sm font-medium dark:text-gray-200">{{ $t('This file type cannot be previewed here.') }}</p>
            <a :href="attachment.path" :download="attachment.name" class="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline">{{ $t('Download') }}</a>
        </div>
        ...
        <template v-else>
            ...
            <!-- Toolbar -->
            <div class="flex items-center gap-2 px-3 sm:px-4 py-2 bg-gray-900 border-b border-white/10 flex-shrink-0 overflow-x-auto">
                <div class="flex items-center gap-1 p-1 rounded-full bg-white/5 flex-shrink-0">
                    <button type="button" @click="drawTool = 'view'" :class="drawTool === 'view' ? 'bg-white text-gray-900' : 'text-white/70 hover:bg-white/10'" class="flex items-center gap-1.5 h-8 px-3 rounded-full text-xs font-medium transition-colors" :title="$t('View / scroll all pages')">
                    ...
                    <button type="button" @click="toggleSketch" :class="['pen','highlighter','eraser'].includes(drawTool) ? 'bg-white text-gray-900' : 'text-white/70 hover:bg-white/10'" class="flex items-center gap-1.5 h-8 px-3 rounded-full text-xs font-medium transition-colors" :title="$t('Sketch')">
                        <svg viewBox="0 0 24 24" fill="none" class="w-4 h-4"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1 1 0 000-1.41l-2.34-2.34a1 1 0 00-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z" fill="currentColor"/></svg>
                        <span class="hidden sm:inline">{{ $t('Sketch') }}</span>
                    </button>
                    <button type="button" @click="toggleTextTool" :class="drawTool === 'text' ? 'bg-white text-gray-900' : 'text-white/70 hover:bg-white/10'" class="flex items-center gap-1.5 h-8 px-3 rounded-full text-xs font-medium transition-colors" :title="$t('Text')">
                        <svg viewBox="0 0 24 24" fill="none" class="w-4 h-4"><rect x="3" y="3" width="18" height="18" rx="3" stroke="currentColor" stroke-width="1.6"/><path d="M8 8h8M12 8v8" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
            ...
            <!-- Sketch sub-tools -->
            <div v-if="drawTool === 'pen' || drawTool === 'highlighter' || drawTool === 'eraser'" class="flex items-center justify-center gap-2 flex-wrap px-4 py-2 bg-gray-900 border-b border-white/10 flex-shrink-0">
            ...
            <div class="flex-1 flex min-h-0">
                ...
                <div class="flex-1 min-w-0 flex flex-col bg-gray-900">
                    <div class="flex-1 min-h-0" :class="drawTool === 'view' ? '' : 'px-2 sm:px-4 py-3 sm:py-6'" @wheel="handleDrawWheel">
                        <div ref="drawScroll" class="pdf-stage-scroll mx-auto w-full h-full" :style="drawTool === 'view' ? '' : 'max-width: 880px;'">
                            <div ref="drawStage" class="modal-pop__stage relative bg-white overflow-hidden mx-auto w-full" :class="[{ 'stage-transitioning': pageTransitioning }, drawTool === 'view' ? '' : 'rounded shadow-2xl']">
                    ...
                    <!-- Page navigation -->
                    <div v-if="totalPdfPages > 1" class="flex items-center justify-center gap-3 px-4 py-2.5 bg-gray-900 border-t border-white/10 flex-shrink-0">
                        <button type="button" @click="goToDrawPage(-1)" :disabled="currentDrawPage <= 1" class="w-8 h-8 rounded-full flex items-center justify-center bg-white/10 text-white disabled:opacity-30 disabled:cursor-not-allowed">
                        ...
                        <span class="text-xs text-white/80 font-medium min-w-[80px] text-center">{{ $t('Page') }} {{ currentDrawPage }} / {{ totalPdfPages }}</span>
                ...
                <aside v-if="showDocumentNotes" class="w-full max-w-[340px] flex-shrink-0 flex flex-col bg-gray-800 border-l border-white/10">
                    ...
                    <div class="flex-1 overflow-y-auto divide-y divide-white/10">
                        ...
                        >
                            <svg viewBox="0 0 24 24" fill="none" class="w-3.5 h-3.5 flex-shrink-0 text-blue-400"><path d="M4 20h4l10-10-4-4L4 16v4z" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/><path d="M14 6l4 4" stroke="currentColor" stroke-width="1.7"/></svg>
                            <div class="flex-1 min-w-0">
                                <div class="text-xs font-medium truncate text-white/90">
                                    {{ version.isOriginal ? $t('Original document') : annotatedVersionLabel(version.name) }}
                                </div>
                                <div class="text-[11px] text-white/50">
                                    {{ moment(version.created_at).format('MMM D, YYYY [at] h:mm A') }}
                                </div>
                            </div>
                            <span v-if="attachment && attachment.id === version.id" class="text-[10px] font-semibold text-blue-400 flex-shrink-0">{{ $t('Viewing') }}</span>
                        </button>
    ...
    export default {
        ...
        methods: {
            ...
            async loadTeamMembers() {
                try {
                    const { data } = await axios.get(this.route('task.other.data', {
            ...
            openAttachment(attachment, pushUrl = true) {
                ...
                if (pushUrl && typeof window !== 'undefined' && window.history?.replaceState) {
                    window.history.replaceState({}, '', this.route('task.attachment.view', {
             ...
             */
            closeViewer() {
                if (this.hasUnsavedAnnotations && !window.confirm(this.$t('You have notes that were never saved. Leave anyway?'))) {
                    return;
                }
                this.leaveViewer();
            },
