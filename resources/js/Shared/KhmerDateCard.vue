    ...
    <div v-if="detail" :class="[
        ...
        <div class="relative flex items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
                ...
                <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium text-gray-600 khmer-lunar-text">
                ...
                <div class="mt-2.5 flex flex-wrap items-center gap-1.5">
                    <span v-if="detail.isFullMoon" class="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-800 ring-1 ring-amber-200">
                        {{ $t('Full Moon') }}
                    </span>
                    <span v-else-if="detail.isNewMoon" class="rounded-full bg-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-700 ring-1 ring-slate-300">
                        {{ $t('New Moon') }}
                    </span>
                    <span v-if="detail.isSilaDay" class="rounded-full bg-violet-100 px-2.5 py-1 text-xs font-semibold text-violet-800 ring-1 ring-violet-200">
                        {{ $t('Precept Day') }}
                    </span>
                    <span
                ...
                <p v-if="!compact && showCountdown && detail.daysToNewYear > 0 && detail.daysToNewYear <= 60" class="mt-3 text-xs font-medium text-gray-500 khmer-lunar-text">
