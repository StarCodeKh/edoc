<template>
    <div
        class="layout-app"
        :class="[current_mode, $page.props.project ? 'project' : 'main', $page.component.replace('/', '_')]"
        :dir="dir"
        :style="[
            $page.props.project && $page.props.project.background
                ? {
                      backgroundColor: $page.props.project.background.bg,
                      backgroundImage: 'url(' + $page.props.project.background.image + ')',
        ...
        <div class="md:flex md:flex-col">
            <div class="md:h-screen md:flex md:flex-col">
                <div class="md:flex md:shrink-0">
                    ...
                    >
                                ...
                                <div class="flex gap-3 select-none top_bar__menu">
                                    ...
                                    <div class="t__l__wrapper" v-click-outside="closeTopMenus">
                                        <div class="mobile__menu__top bg-[#a6c5e229]" @click="togglePanel('more')">
                                            ...
                                            <icon class="ml-2 w-4 h-4 text-white" name="arrow-down" />
                                        </div>
                                        <div class="tl_menu_list hidden" :class="{ mobile: show__menu__list }">
                                            <div
                                                ...
                                                "
                                                @click="toggleSubMenu('menu_recent')"
                                            >
                                                <span class="text-white">{{ $t('Recently Viewed') }}</span>
                                                <icon class="ml-2 w-4 h-4 text-white" name="arrow-down" />
                                                <top-project-menu
                                                    v-if="visible.menu_recent"
                                                    filter="recent"
                                                    tabindex="-1"
                                                />
                                            </div>
                                            <div
                                                ...
                                                "
                                                @click="toggleSubMenu('menu_workspace')"
                                            >
                                                <span class="text-white">{{ $t('My Workspaces') }}</span>
                                                <icon class="ml-2 w-4 h-4 text-white" name="arrow-down" />
                                        ...
                                        <div
                                            v-if="
                                                this.$page.props.auth.user.role.create_project ||
                                                this.$page.props.auth.user.role.create_workspace
                                            "
                                            class="__creation"
                                            v-click-outside="
                                                () => {
                                                    visible.menu_create = false;
                                                }
                                            "
                                                ...
                                                <div tabindex="-1" class="m__area">
                                                    <ul role="menu" class="">
                                                        <li v-for="create in creations" class="group">
                                                            <div
                                                                v-if="create.condition"
                                                                class="c__1"
                                                                @click="visible[create.visible] = true"
                                                            >
                                                                <div class="c__2">
                                                                    <div class="c__3">
                        ...
                        <div class="top-search relative" v-click-outside="clearSearch">
                            ...
                            />
                            <kbd
                                class="hidden lg:block absolute end-2.5 top-1/2 -translate-y-1/2 px-1.5 py-0.5 rounded-md border border-gray-200 bg-gray-50 text-[10px] font-sans font-semibold text-gray-400 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400"
                            >
                                {{ searchShortcutLabel }}
                            </kbd>

                            <div
                                v-if="search_loading || hasSearchResults || noSearchResults"
                                class="search_result absolute left-0 right-0 top-full mt-2 z-30"
                            >
                                <div
                                    v-if="search_loading"
                                    class="flex justify-center items-center p-3 rounded-xl bg-white shadow-lg ring-1 ring-black/5 dark:bg-gray-700 dark:ring-white/10"
                                >
                                    <svg
                                        aria-hidden="true"
                                ...
                                <div
                                    ...
                                    class="search__result overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-black/5 divide-y divide-gray-100 dark:bg-gray-700 dark:ring-white/10 dark:divide-gray-600"
                                >
                                    <div class="sr__projects" v-if="search_result.projects.length">
                                        <h4
                                            class="sr__title px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-gray-400"
                                        >
                                            {{ $t('Projects') }}
                                        </h4>
                                        <ul
                                            class="pb-2 text-sm text-gray-700 dark:text-gray-200 max-h-56 overflow-y-auto"
                                        >
                                            <li v-for="s_i in search_result.projects" :key="'p_' + s_i.id">
                                                <Link
                                                    class="block px-3 py-2 truncate hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                                ...
                                <div
                                    v-else
                                    class="px-3 py-4 text-center text-sm rounded-xl bg-white text-gray-500 shadow-lg ring-1 ring-black/5 dark:bg-gray-700 dark:text-gray-300 dark:ring-white/10"
                                >
                                    {{ $t('No results found.') }}
                                </div>
                            </div>
                        ...
                        <div class="placement-top-right gap-3">
                            <div class="tracker" v-if="this.counter.timer && this.activeTimerString">
                                ...
                                <Link
                                    :href="
                                        this.route('projects.view.board', {
                                            uid: this.counter.timer.task.project_id,
                                            task: this.counter.timer.task.slug || this.counter.timer.task.id,
                                        })
                                    "
                ...
                <div class="md:flex md:flex-grow md:overflow-hidden">
                    ...
                    <div
                        v-if="!enable_sidebar"
                        class="top-0 left-0 w-4 h-full left__bar hidden md:block"
                        @click="enable_sidebar = true"
                    >
                        <div class="w-4 h-4 arr"><icon class="w-4 h-4" name="arrow-right" /></div>
                    </div>
                    <workspace-menu
                        v-if="$page.props.project || $page.props.workspace"
                        class="sidebar app-sidebar shrink-0 md:w-60 overflow-y-auto"
                        @enableSidebar="enable_sidebar = false"
                        :class="{ __hide: !enable_sidebar, 'is-open': mobile_nav }"
                        :style="[
                            $page.props.project && $page.props.project.background
                                ? { backgroundColor: $page.props.project.background.side }
                                : {},
                        ]"
                    />
                    ...
                    <main-menu
                        v-else-if="$page.props.auth.user.role.slug === 'admin'"
                        class="sidebar app-sidebar shrink-0 md:w-60 overflow-y-auto"
                ...
                <create-workspace
                    v-if="visible.create_workspace"
                    @create-workspace="visible.create_workspace = false"
                />
            </div>
        </div>
    </div>
...
export default {
    ...
    data() {
        return {
            creations: [
                {
                    name: 'Project',
                    visible: 'project_create',
                    icon: 'project',
                    condition: !!this.$page.props.auth.user.role.create_project,
                    details: 'After creating project, you will be able to manage your tasks on board.',
                },
                {
                    name: 'Workspace',
                    visible: 'create_workspace',
                    condition: !!this.$page.props.auth.user.role.create_workspace,
                    icon: 'workspace',
                    details: 'After creating project, you will be able to manage your tasks on board.',
                },
            ],
    ...
    computed: {
        ...
        hasSidebar() {
            return (
                !!(this.$page.props.project || this.$page.props.workspace) ||
        ...
        activeLanguage() {
            return this.availableLanguages.find((l) => l.code === this.locale) || { code: 'en', name: 'English' };
        },
    },
    // $page.props.counter
    watch: {
    ...
    methods: {
        ...
        stopTracker() {
            axios
                .post(this.route('task.timer.stop'), { duration: this.counter.seconds, id: this.counter.timer.id })
                .then((response) => {
        ...
        doSearch(e) {
            ...
            if (search.length > 2) {
                ...
                vm.search_timer = setTimeout(function () {
                    axios
                        .post(vm.route('json.task.search', { q: search }))
                        .then((response) => {
