<template>
    ...
    >
        ...
        <div v-else class="flex flex-col max-h-[80vh] overflow-hidden">
            <!-- Header -->
            <div
                class="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-indigo-50 to-blue-50 border-b border-gray-200"
            >
                <div class="flex items-center gap-2">
                    ...
                    <span
                        ...
                        class="ml-2 px-2 py-0.5 text-xs font-bold text-white bg-indigo-600 rounded-full"
                    >
                        {{ activeFiltersCount }}
                    </span>
                </div>
                <button
                    @click="$emit('boardFilter')"
            ...
            <div class="flex-1 overflow-y-auto px-4 py-3 space-y-4">
                ...
                <div v-if="enable_options.includes('due')" class="filter-section">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <button
                            v-if="hasActiveDueFilters"
                            @click="clearFilterType('due')"
                            class="text-xs text-indigo-600 hover:text-indigo-700 font-medium"
                        >
                            {{ $t('Clear') }}
                        </button>
                ...
                <div v-if="enable_options.includes('label')" class="filter-section">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <button
                            v-if="hasActiveLabelFilters"
                            @click="clearFilterType('label')"
                            class="text-xs text-indigo-600 hover:text-indigo-700 font-medium"
                        >
                            {{ $t('Clear') }}
                        </button>
                    ...
                    <ul class="flex flex-col gap-2 max-h-48 overflow-y-auto">
                        <li v-for="(project, project_index) in workspaceProjects || []" :key="project_index">
                            ...
                            >
                                ...
                                />
                                <div
                                    class="project__color w-5 h-5 rounded-full flex-shrink-0"
                                    :style="[
                                        project.background
                                            ? { background: 'url(' + project.background.image + ')' }
                                            : {},
                                    ]"
                                ></div>
...
export default {
    ...
    computed: {
        filteredUsers() {
            ...
            return this.users.filter((user) => user && user.name && user.name.toLowerCase().includes(search));
        },
        filteredLabels() {
            ...
            return this.labels.filter((label) => label && label.name && label.name.toLowerCase().includes(search));
        },
        activeFiltersCount() {
            let count = 0;
            if (this.form.user) {
    ...
    methods: {
        ...
        getData() {
            if (this.workspace) {
                axios
                    .get(this.route('workspace.other.data', { workspace_id: this.workspace.id }))
                    .then((response) => {
                ...
                axios
                    .get(this.route('json.project.filter.data', this.project.id))
                    .then((response) => {
                        const data = response.data || {};
