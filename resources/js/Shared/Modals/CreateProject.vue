<template>
    <div
        class="fixed inset-0 z-[200] flex items-start sm:items-center justify-center sm:justify-start bg-black/40 p-3 sm:pl-[250px] overflow-y-auto create__project"
        @click.self="$emit('createProject')"
    >
        <div
            class="flex flex-col w-full max-w-[320px] sm:max-w-[340px] max-h-[calc(100vh-24px)] my-auto rounded-[8px] bg-white shadow overflow-hidden"
            v-if="!loading"
        >
            <!-- Sticky header -->
            <div
                class="flex items-center justify-between gap-1 py-3 px-3 sticky top-0 bg-white z-10 border-b border-gray-100 shrink-0"
            >
                <div class="flex"></div>
            ...
            <div class="flex flex-col gap-3 overflow-y-auto px-3 pb-3 min-h-0">
                <div class="flex justify-center">
                    <div
                        class="w-[70%] h-[100px] p-3 flex rounded justify-center"
                        :style="{
                            backgroundImage: 'url(' + project.color.image + ')',
                            ...
                            <ul class="grid grid-cols-5 gap-2.5 auto-rows-max">
                                <li v-for="color in selected_backgrounds" :key="color.id" class="flex justify-center">
                                    <button
                                        ...
                                        class="w-10 h-8 flex items-center justify-center rounded transition-opacity hover:opacity-80"
                                        :style="{
                                    ...
                                    >
                                        <icon
                                            v-if="project.color.id === color.id"
                                            name="tick_check"
                                ...
                                <li v-if="selected_backgrounds.length < backgrounds.length" class="flex justify-center">
                                    <button
                                        @click="showAllBackgrounds()"
                                        class="w-10 h-8 flex items-center justify-center rounded bg-black hover:bg-gray-800 transition-colors"
                                        type="button"
                                    >
                ...
                <div class="flex">
                    <label class="flex flex-col w-full">
                        ...
                        <select-input v-model="project.workspace_id" class="w-full">
                            <option v-for="(workspace, wi) in workspaces" :key="wi" :value="workspace.id">
                                {{ workspace.name }}
                            </option>
                        </select-input>
                    </label>
                ...
                <div class="flex">
                    ...
                    <div class="ms-1 text-sm">
                        <label for="helper-checkbox" class="font-medium text-[13px] text-gray-900 dark:text-gray-300"
                            >Visible tasks for assigned people
                            <small
                                >(Enabling this the tasks will be visible only for the admin and assigned people)</small
                            ></label
                        >
                    </div>
                </div>
                <div class="flex">
                    <label class="w-full flex flex-col">
                        <div>
                            {{ $t('Project Details') }} <small>({{ $t('optional') }})</small>
                        </div>
                        <textarea
            ...
            <div class="flex px-3 py-3 sticky bottom-0 bg-white z-10 border-t border-gray-100 shrink-0">
                <button
                    class="bg-indigo-600 w-full text-white p-[9px] rounded disabled:opacity-50"
        ...
        <div
            v-if="showAlert"
            class="fixed inset-0 z-[300] flex items-center justify-center bg-black/40 p-3"
        ...
        >
            <div class="bg-white rounded-[8px] shadow-lg w-full max-w-[280px] p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between">
                    ...
                    <div
                        @click="closeAlert()"
                        class="flex hover:bg-gray-200 cursor-pointer rounded w-6 h-6 justify-center items-center"
                    >
    ...
    methods: {
        async getData() {
            ...
            try {
                ...
                console.error(
                    '[debug] CreateProject workspace.board.index FAILED:',
            ...
            if (this.$page.props.workspace || this.$page.props.project) {
                this.project.workspace_id = this.$page.props.workspace
                    ? this.$page.props.workspace.id
                    : this.$page.props.project
                      ? this.$page.props.project.workspace_id
                      : '';
            }
