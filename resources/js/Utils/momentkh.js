/* eslint-disable */
/**
 * Khmer (Chhankitek / ចន្ទគតិ) calendar engine — vendored, dependency-free.
 *
 * Source: @thyrith/momentkh v3.0.3 — https://github.com/ThyrithSor/momentkh
 * MIT License, Copyright (c) 2018 ThyrithSor
 *
 * Vendored (instead of installed) so the calendar keeps working without an
 * extra npm dependency. The CommonJS build below is untouched; only the module
 * wrapper at the top and the ESM re-exports at the bottom were added.
 *
 * Do not edit the body by hand — re-vendor from npm if it ever needs updating.
 */

const __momentkh = (function () {
    ...
    const SolarMonthAbbreviationNames = ['មក', 'កម', 'មន', 'មស', 'ឧស', 'មថ', 'កដ', 'សហ', 'កញ', 'តល', 'វក', 'ធន'];
    ...
    const AnimalYearNames = ['ជូត', 'ឆ្លូវ', 'ខាល', 'ថោះ', 'រោង', 'ម្សាញ់', 'មមី', 'មមែ', 'វក', 'រកា', 'ច', 'កុរ'];
    ...
    const WeekdayNames = ['អាទិត្យ', 'ចន្ទ', 'អង្គារ', 'ពុធ', 'ព្រហស្បតិ៍', 'សុក្រ', 'សៅរ៍'];
    ...
    /**
     * Validates JavaScript Date objec
    ...
    function isKhmerLeapMonth(beYear) {
        ...
        return (bodithey === 24 && boditheyNextYear === 6) || bodithey >= 25 || bodithey < 6;
    }
    ...
    function getSunInfo(jsYear, sotin) {
        ...
        // Sun inauguration
        const sunInaugurationAsLibda = kaen <= 5 ? sunAverageAsLibda - pholAsLibda : sunAverageAsLibda + pholAsLibda;
        return {
    ...
    function getNewYearInfo(jsYear) {
        const sotins = has366Days(jsYear - 1) ? [363, 364, 365, 366] : [362, 363, 364, 365];
    ...
    function gregorianToKhmerInternal(year, month, day, hour = 0, minute = 0, second = 0, isSearching = false) {
        ...
        let khmerDayNumber = 0; // 0-29 forma
        ...
        // Convert dayNumber to day/moonPhase forma
    ...
    function khmerToGregorian(day, moonPhase, monthIndex, beYear) {
        ...
        if (candidates.length > 1) {
            ...
            // If we have a unique closest candidate, return i
    ...
    function getNewYearFullInfo(ceYear) {
        ...
        const lerngSakMoment = new Date(lerngSakDate.year, lerngSakDate.month - 1, lerngSakDate.day, 0, 0, 0); // Midnigh
        const result = {
            newYearMoment,
            lerngSakMoment,
            newYearInfo: {
    ...
    // Utility for creating date from Date objec
