...
@section('content')
    <img src="{{ asset('images/errors/500.svg') }}" class="error-art" alt="Internal server error illustration">

    <span class="error-code">{{ __('Error 500') }}</span>

    <h1 class="error-title">{{ __('Internal Server Error!') }}</h1>

    <p class="error-text">{{ __("Something went wrong on our end. We're working to fix it, please try again shortly.") }}</p>
